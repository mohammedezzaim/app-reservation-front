...
export class ChartsComponent implements OnInit {
  ...
  constructor(private locationService: LocationService, protected authService: AuthService,private categorieVoitureService:CategorieVoitureService,private voitureService:VoitureService) {
  }
...
//        {name: 'HIV/AIDS', y: 2, color: '#393e46'},
  ...
  generatePieChart(): void {
    this.categorieVoitureService.getAll().subscribe(categories => {
      const observables = categories.map(category => {
        return this.voitureService.getByCategorieVoitureLibelleAndAgenceUsername(category.libelle,this.authService.username);
      });
